...
size_batch=32  # mini-batch size for training and testing, must be square of an integer
...
def data_load():
    ...
    height = 128
    j = 0
    folder_labeled_images = "/home/ganch/GAN/Data/labeled1/"
    for i in range(20):
        if (i == 0):
...
if __name__ == '__main__':
    ...
    image_path = "/home/ganch/GAN/OptimizerTest/"
    num_epochs = 26
    sess = tf.Session()
    width = 128
    height = 128
    with sess.as_default():
        ...
        loss_EG_OP = tf.train.GradientDescentOptimizer(learning_rate = 0.0002).minimize(loss_EG)
        loss_Dz_OP = tf.train.GradientDescentOptimizer(learning_rate = 0.0002).minimize(loss_Dz)
        loss_Di_OP = tf.train.GradientDescentOptimizer(learning_rate = 0.0002).minimize(loss_Di)
